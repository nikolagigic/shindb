      ...
      getMany: (ids: DocId[]) => {
        return this.processBatchOperation(
          ...
          async (batch) => {
            ...
            return { status: 0, data: response } as BatchResponse<T, 'getMany'>;
          },
          (results, batchResult) => {
            (results.data as RowWithId<T>[]).push(
              ...(batchResult.data as RowWithId<T>[])
            );
          }
        );
      },
...
export const { setup, disconnect, collection } = autobindStatics(ClientV2);
